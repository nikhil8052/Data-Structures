...
double median3(int e1 , int e2 , int e3){
   int sum=e1+e2+e3;
   int allmax=max(e1,max(e2,e3));
   int allmin=min(e1,max(e2,e3));
...
double median4(int a , int b , int c , int d ){
    int allsum = a+b+c+d;
    int allmax=max(a,max(b,max(c,d)));
    int allmin=min(a,min(b,min(c,d)));
...
double medianArr(vector<int> arr, int  n) {
    if (n == 0) return -1;
    if (n % 2 == 0) {
        n = n / 2;
